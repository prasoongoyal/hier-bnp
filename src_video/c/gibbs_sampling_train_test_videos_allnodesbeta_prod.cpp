...
class Model {
  ...
  Model(float alpha, float GAMMA, int num_levels, int branching_factor, 
    ...
    if (isTrain) {
      beta_internal = new double[num_paths * num_features];
      best_beta_internal = new double[num_paths * num_features];
      beta = new double[num_paths * num_features];
      best_beta = new double[num_paths * num_features];

      for (int p=0; p<num_internal_nodes; p++) {
        for (int f=0; f<num_features; f++) {
          beta_internal[p * num_features + f] = alpha;
        }
      }
      for (int p=0; p<num_paths; p++) {
        for (int f=0; f<num_features; f++) {
  ...
  ~Model() {
    delete [] beta_internal;
    delete [] best_beta_internal;
    delete [] beta;
    delete [] best_beta;
  }
  ...
  void sample_Zdn(double* W_dn, int d, int n, int num_words_in_doc, double temp,
    ...
    for (int p=0; p<num_paths; p++) {
      ...
      int pp = (p - 1) / branching_factor;
      while (pp > 0) {
        likelihoods[p] += likelihood_gaussian(W_dn, beta_internal + pp * num_features, sigma);
        pp = (pp - 1) / branching_factor;
      }
      likelihoods[p] += likelihood_gaussian(W_dn, beta_internal + 0 * num_features, sigma);
  ...
  void sample_beta(double* W, int* Z, double sigma) {
    ...
    */
    ...
    memset(sumW_internal, 0, num_internal_nodes * num_features * sizeof(double));
    ...
    */
    ...
    for (int p=0; p<num_paths; p++) {
      int parent_id = (p + num_internal_nodes - 1) / branching_factor;
      for (int f=0; f<num_features; f++) {
        beta_sum_children[parent_id * num_features + f] += beta[p * num_features + f];
      }
    }
    for (int p=num_internal_nodes; p>0; p--) {
      int parent_id = (p - 1) / branching_factor;
      for (int f=0; f<num_features; f++) {
        double mu_pf = (beta_internal[parent_id * num_features + f] + 
            beta_sum_children[p * num_features + f]) / (1 + branching_factor);
        beta_internal[p * num_features + f] = mu_pf + gsl_ran_gaussian(GSL_RNG, sigma_p);
        beta_sum_children[parent_id * num_features + f] += 
            beta_internal[p * num_features + f];
      }
    }
    // sample root beta
    for (int f=0; f<num_features; f++) {
      double mu_pf = (alpha + beta_sum_children[0 * num_features + f]) / 
          (1 + branching_factor);
  ...
  void update_best_configuration(double* W, int iter, int* countsPerDoc, int* Z, 
    ...
    if (curr_log_likelihood > best_log_likelihood) {
      ...
      if (isTrain) {
        memcpy(best_beta, beta, num_paths * num_features * sizeof(double));
        memcpy(best_beta_internal, beta_internal, num_paths * num_features * sizeof(double));
      }
    }
  }
